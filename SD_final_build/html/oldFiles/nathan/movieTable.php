...
echo '<a href="DBsortAName.php?search=' . $searchText. '"> Actor Sort </a> 
 ...
 <a href="movieTable.php?search=' . $searchText. '"> Movie Table </a>';
	...
	FROM Relations,Movie,Director
	
	WHERE Movie.TITLE = Relations.MovieTitle AND Director.ID = Relations.DirectorID
	AND (lower(Director.Name) LIKE '%".$searchText."%' OR 
	lower(Movie.TITLE) LIKE '%".$searchText."%' OR
	 lower(Movie.Genre) LIKE '%".$searchText."%' OR 
	 lower(Movie.Rating) LIKE '%".$searchText."%' OR
	 lower(Movie.Date) LIKE '%".$searchText."%')
	";	
